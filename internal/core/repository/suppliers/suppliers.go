...
func (supplier *Suppliers) Insert(
	...
	return supplier.conn.Transaction(func(tx *gorm.DB) error {
		if err := db.SafeWriteQuery(
			ctx, tx, InsertIntoSuppliers, supp.Name, supp.Email,
		); err != nil {
			return err
		}
...
func (supplier *Suppliers) GetLimit(
	...
	if err := supplier.conn.
		Table(domain.SuppliersTable).
		Find(&_suppliers).Limit(limit).Error; err != nil {
		return nil, err
	}
...
func (supplier *Suppliers) GetByPhone(
	...
	if err := supplier.conn.
		Table(domain.SuppliersTable).
		Where("email = ?", email).First(&_supplier).Error; err != nil {
		return nil, err
	}
