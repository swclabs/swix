...
type IProductService interface {
	...
	GetProductsLimit(ctx context.Context, limit int) ([]dto.ProductSchema, error)
	...
	CreateProduct(ctx context.Context, products dto.Product) (int64, error)
	...
	// Returns an error if any issues occur during the insertion process.
	CreateSuppliers(ctx context.Context, supplierReq dto.Supplier) error

	// InsertIntoInventory adds a product to the Inventories.
	// ctx is the context to manage the request's lifecycle.
	// product contains the inventories product details to be added.
	// Returns an error if any issues occur during the insertion process.
	InsertIntoInventory(ctx context.Context, product dto.InventoryDetail) error
	...
	FindDeviceInInventory(ctx context.Context, deviceSpecs dto.InventoryDeviceSpecs) (*dto.Inventory, error)
	...
	UpdateProductInfo(ctx context.Context, product dto.UpdateProductInfo) error
	...
	// Returns a slice of ProductSchema objects and an error if any issues occur during the retrieval process.
	Search(ctx context.Context, keyword string) ([]dto.ProductSchema, error)
	...
	GetAllStock(ctx context.Context, page int, limit int) (*dto.StockInInventory, error)
